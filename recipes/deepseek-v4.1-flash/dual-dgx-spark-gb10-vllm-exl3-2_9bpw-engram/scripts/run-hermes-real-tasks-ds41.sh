#!/usr/bin/env bash
# Hermes real-task benchmark, DS41/DS variant of
# execution/benchmarks/run-hermes-real-tasks-ab.sh (qwen/ds original).
# Runs on the hermes gateway host; all model endpoints are LAN-reachable.
#
#   run-hermes-real-tasks-ds41.sh [--treatment ds41|ds|ds41-ds]   (default ds41-ds)
#
...
ARTIFACT_BASE="${ARTIFACT_BASE:-$HERMES_HOME/benchmarks/real-tasks-ds41}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
ROOT="$ARTIFACT_BASE/$RUN_ID"
JOBS_FILE="$HERMES_HOME/cron/jobs.json"

HEAD_LAN="${HEAD_LAN:-192.0.2.20}"
DS41_PROVIDER="local-vllm-ds41"
DS41_MODEL="DeepSeek-v4.1-Flash-EXL3"
DS41_METRICS_URL="http://${HEAD_LAN}:18890/metrics"
DS_PROVIDER="local-vllm-private-ds"
DS_MODEL="deepseek-v4-flash-0731"
DS_METRICS_URL="http://${HEAD_LAN}:8890/metrics"
...
case "$TREATMENT" in ds41|ds|ds41-ds) ;; *) echo "bad --treatment" >&2; exit 2 ;; esac
...
run_case() {
    ...
    if [[ "$treatment" == "ds41" ]]; then
        provider="$DS41_PROVIDER"; model="$DS41_MODEL"; metrics_url="$DS41_METRICS_URL"
    else
...
write_summary() {
    ...
    jq -r '"# Hermes real-task benchmark (ds41/ds)\n\n"
        + "Run: `" + .run_id + "`\n\n"
...
main() {
    ...
    curl -fsS "http://${HEAD_LAN}:8890/v1/models" > "$ROOT/ds-model.before.json" || true
    curl -fsS "http://${HEAD_LAN}:18890/v1/models" > "$ROOT/ds41-model.before.json" || true

    log "Creating isolated profile $BENCH_PROFILE"
    "$HERMES_BIN" profile create "$BENCH_PROFILE" \
        --clone-from "$SOURCE_PROFILE" \
        --no-alias \
        --description 'Temporary isolated profile for real-task DS41/DeepSeek benchmark' \
        > "$ROOT/profile-create.log"
    PROFILE_CREATED=1

    "$HERMES_BIN" -p "$BENCH_PROFILE" config set --force providers.local-vllm-ds41 \
        '{"name":"local-vllm-ds41","base_url":"http://'"${HEAD_LAN}"':18890/v1","api_key":"no-key-required","default_model":"DeepSeek-v4.1-Flash-EXL3","extra_body":{"chat_template_kwargs":{"enable_thinking":true},"reasoning_effort":"low"}}' \
        >> "$ROOT/profile-config.log"
    "$HERMES_BIN" -p "$BENCH_PROFILE" config set --force providers.local-vllm-private-ds \
        '{"name":"local-vllm-private-ds","base_url":"http://'"${HEAD_LAN}"':8890/v1","api_key":"not-needed","default_model":"deepseek-v4-flash-0731","extra_body":{"chat_template_kwargs":{"enable_thinking":true},"reasoning_effort":"low"}}' \
        >> "$ROOT/profile-config.log"

    # Task ids select entries from the local jobs file; resolve at runtime and
    # never hard-code production identifiers in a public tree.
    local ids=(${TASK_IDS:-replace-with-task-id-1 replace-with-task-id-2 replace-with-task-id-3})
    local slugs=(codex-quota-reset daily-stock-price-monitor hermes-version-check)
    local i t
    if [[ "$TREATMENT" == "ds41-ds" ]]; then
        for i in 0 1 2; do
            run_case "${ids[$i]}" "${slugs[$i]}" ds41

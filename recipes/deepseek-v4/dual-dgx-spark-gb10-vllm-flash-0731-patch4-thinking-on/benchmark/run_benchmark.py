...
def load_manifest() -> dict[str, Any]:
    ...
    endpoint_variables = {
        "online_ds": "CLAUDE_DS_BASE_URL",
        "offline_ds": "PRIVATE_DS_BASE_URL",
        "qwen_local": "QWEN_LOCAL_BASE_URL",
    }
    for treatment, variable in endpoint_variables.items():
        if treatments[treatment].get("base_url_env") != variable or "base_url" in treatments[treatment]:
            raise InfrastructureError(f"endpoint contract mismatch: {treatment}")
...
def provider_spec(treatment: str, manifest: dict[str, Any] | None = None) -> dict[str, str]:
    ...
    if treatment in TREATMENTS:
        ...
        variable = str(contract["base_url_env"])
        base_url = os.environ.get(variable, "").strip()
        if not base_url:
            raise InfrastructureError(f"{variable} is required for {treatment}")
...
def controlled_files(toolchain: Path) -> list[Path]:
    files = [SCRIPT_PATH, REPORTER_PATH, MANIFEST_PATH, BENCHMARK_DIR / "run.sh", toolchain / "bin" / "claude"]

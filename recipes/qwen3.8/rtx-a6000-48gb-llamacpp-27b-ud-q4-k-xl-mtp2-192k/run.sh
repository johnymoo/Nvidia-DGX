...
ROOT=$RECIPE_DIR
while [[ ! -x "$ROOT/lab" && "$ROOT" != "/" ]]; do ROOT=$(dirname "$ROOT"); done
...
if [[ $# -eq 0 || "${1:-}" == "--help" ]]; then
  python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); print("supported:", " ".join(sorted(d["operations"])))' "$RECIPE_DIR/recipe.yaml"
  exit 0
fi

...
download_file() {
  ...
  output="${MODEL_ROOT}/${name}"; partial="${output}.partial"
  url="https://modelscope.cn/models/${MODEL_REPO}/resolve/${MODELSCOPE_REVISION}/${name}"
  if [[ ! -f "${output}" ]]; then curl --fail --location --retry 12 --retry-all-errors --continue-at - --output "${partial}" "${url}"; mv "${partial}" "${output}"; fi

#!/usr/bin/env bash
# Local-only entry for the GLM-5.3-Flash EXL3 dual-GB10 recipe.
...
case "${1:-help}" in
  validate)
    ...
    bash -n "$SCRIPT_DIR/scripts/prep_remote.sh"
    bash -n "$SCRIPT_DIR/scripts/run-hermes-real-tasks.sh"
    python3 -m py_compile \
      "$SCRIPT_DIR/scripts/bench_prefill_ladder.py" \
      "$SCRIPT_DIR/scripts/bench_concurrent.py" \
      "$SCRIPT_DIR/scripts/summarize.py"
    python3 -c "import json; json.load(open('$SCRIPT_DIR/benchmark-results-20260911.json'))"

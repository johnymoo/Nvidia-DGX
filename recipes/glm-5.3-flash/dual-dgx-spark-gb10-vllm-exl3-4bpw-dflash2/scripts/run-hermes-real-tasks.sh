...
GB10_LAN="${GB10_LAN:-192.0.2.10}"
GLM_PROVIDER="local-vllm-glm53"
GLM_MODEL="GLM-5.3-Flash-EXL3"
GLM_METRICS_URL="http://${GB10_LAN}:18888/metrics"
...
main() {
    ...
    curl -fsS "http://${GB10_LAN}:18888/v1/models" > "$ROOT/glm-model.before.json" || true
    ...
    "$HERMES_BIN" -p "$BENCH_PROFILE" config set --force providers.local-vllm-glm53 \
        '{"name":"local-vllm-glm53","base_url":"http://'"${GB10_LAN}"':18888/v1","api_key":"no-key-required","default_model":"GLM-5.3-Flash-EXL3","extra_body":{"chat_template_kwargs":{"enable_thinking":true},"reasoning_effort":"low"}}' \

...
- Attach the complete rear frame to the four-sided main body with two integral rigid upper locating tongues, a three-sided alignment lip, and two integral 20 mm PETG cantilever latches. Each latch uses a single insertion ramp, vertical retaining shoulder, 1.8 mm base keeper lip, direct underside release access, and a 3 mm anti-relock travel state; rear-fan service may require disconnecting the GB10 rear cables first.
...
- R2 changes the main enclosure split to a one-piece rounded top-and-side U cover plus a front-loading ribbed bottom plate. Continuous chamfered base tongues engage three self-supporting capture-rail segments per side, each 34 mm long with real 2 mm Y-direction lead-in tapers and debris-relief gaps.
- R2 uses a snap-on front fan bezel with an integral 25 mm fan plenum, three-sided alignment lip, two upper locating tongues, two 20 mm PETG cantilever latches, two GB10 front stops, and two base stops. Each lower latch releases separately into a 3 mm anti-relock state; the bezel locks axial base movement but does not carry vertical device load.
...
- Target 10-12 mm outer U-cover radii, 3-5 mm exposed-edge radii, 3.2-3.6 mm shell walls with local ribs, and at least 80% CAD-projected front-grille open area with no printed clear aperture above 12 mm.
- Make the display pod installable on either side with two short T keys, 7 mm of downward locking travel, and one 22 mm vertical flex latch requiring no more than 1.4 mm outward release deflection; do not use a long side rail. Cover the unused mirrored interface with a snap-on blank, and route the harness continuously from the protected base channel through side-wall guide rails and a dedicated port into the bottom of the pod. Default presentation remains on the left.

...
<div class="decision-strip"><article class="private"><small>默认私有线路</small><strong>Private Flash · High</strong><span>{pct(quality['private_flash']['high']['macro_score'])} 宏平均 · {latency['private-high']['ttft_seconds']:.3f}s TTFT</span></article><article class="pro"><small>最高精度</small><strong>Online Pro · High</strong><span>{pct(quality['online_pro']['high']['macro_score'])} 宏平均</span></article><article><small>True Max 适用边界</small><strong>Private Flash · Max</strong><span>{latency['private-max']['ttft_seconds']:.3f}s TTFT · 全量精度未完成</span></article></div></div></header>

<section id="quality"><div class="shell"><div class="section-head"><div><span class="section-index">01 / QUALITY</span><h2>精度对比</h2><p>仅展示 effort 契约已验证且完整完成的七个处理组，每组 n=2。</p></div></div><div class="quality-grid"><div class="chart"><h3>可执行宏平均</h3>{''.join(quality_bars)}</div><div class="table-wrap"><table><thead><tr><th>处理组</th><th>宏平均</th><th>SQL</th><th>Python</th><th>故障</th><th>波动</th></tr></thead><tbody>{''.join(quality_rows)}</tbody></table></div></div><p class="note" style="margin-top:14px">旧 Private Low/High/Max 的 `reasoning_effort` 被 LiteLLM 静默丢弃，实际均为 High；原 83.3%/90.3%/87.5% 仅是重复波动。True Max 全量质量运行因超长生成终止，不给出误导性精度分数。</p></div></section>

...
def main() -> int:
    ...
    treatments = [
        ("Private Flash", "Low*", quality["private_flash"]["low"], "private"),
        ("Private Flash", "High", quality["private_flash"]["high"], "private"),
        ("Private Flash", "Max", quality["private_flash"]["max"], "private"),
...
<div class="decision-strip"><article class="private"><small>默认私有线路</small><strong>Private Flash · High</strong><span>90.3% 宏平均 · 0.252s TTFT</span></article><article class="pro"><small>最高精度</small><strong>Online Pro · High</strong><span>97.2% 宏平均</span></article><article><small>Private Max 已测</small><strong>Private Flash · Max</strong><span>87.5% 宏平均 · 未超过 High</span></article></div></div></header>

<section id="quality"><div class="shell"><div class="section-head"><div><span class="section-index">01 / QUALITY</span><h2>精度对比</h2><p>九个观测组各 n=2。Flash/private 的歧义 CDC 题在 v1 裁决中排除；Pro v2 使用完整 18 题。</p></div></div><div class="quality-grid"><div class="chart"><h3>可执行宏平均</h3>{''.join(quality_bars)}</div><div class="table-wrap"><table><thead><tr><th>处理组</th><th>宏平均</th><th>SQL</th><th>Python</th><th>故障</th><th>波动</th></tr></thead><tbody>{''.join(quality_rows)}</tbody></table></div></div><p class="note" style="margin-top:14px">* Private Low 与 High 在当前 vLLM tokenizer 中生成相同 prompt；Low 行是历史观测，不是独立 effort 效应。Private Max 使用不同 prompt，已完成两轮质量测试。</p></div></section>

<section id="latency"><div class="shell"><div class="section-head"><div><span class="section-index">02 / PERFORMANCE</span><h2>短请求性能</h2><p>1 次预热、3 次串行 SSE。Private 不是直连 vLLM；所有数值均为客户端端到端观测。</p></div></div><div class="route"><small>Private 实测调用链</small><strong>{esc(private_route_text)}</strong></div><div class="metric-band"><div class="metric private"><small>Private High 经 Portal TTFT</small><b>{latency['private-high']['ttft_seconds']:.3f}s</b></div><div class="metric pro"><small>Pro High 端到端</small><b>{latency['online-pro-high']['response_seconds']:.3f}s</b></div><div class="metric"><small>Online Flash Low 解码</small><b>{latency['online-flash-low']['decode_tokens_per_second']:.1f} tok/s</b></div></div><div class="chart"><div class="latency-list">{''.join(latency_rows)}</div></div><p class="note" style="margin-top:14px">Private 的 0.252s TTFT 包含反向代理、Portal、兼容层和 WireGuard，不代表裸 vLLM engine latency。</p></div></section>
...
<section id="coverage"><div class="shell"><div class="section-head"><div><span class="section-index">05 / TEST COVERAGE</span><h2>本轮覆盖范围</h2><p>“未测”表示本轮没有可比证据，不以其他 effort 或历史结果代填。</p></div></div><div class="table-wrap"><table><thead><tr><th>维度</th><th>本轮纳入</th><th>未覆盖</th><th>状态</th></tr></thead><tbody><tr><th>可执行精度</th><td>Private / Online Flash / Online Pro：Low、High、Max</td><td>Private Low 非独立 prompt</td><td><span class="status pass">完整</span></td></tr><tr><th>串行 SSE 性能</th><td>Private High；Online Flash Low；Online Pro Low/High/Max</td><td>Private Low/Max；Online Flash High/Max</td><td><span class="status fail">部分</span></td></tr><tr><th>Token 与 API 成本</th><td>Online Pro Low/High/Max</td><td>Private 无 API 账单；Flash 未统一计价</td><td><span class="status pass">范围内完整</span></td></tr><tr><th>Agent 聚焦任务</th><td>Online Pro High；Online/Private 历史基线</td><td>不是九组 effort 全矩阵</td><td><span class="status fail">部分</span></td></tr></tbody></table></div></div></section>

<section id="boundary"><div class="shell"><div class="section-head"><div><span class="section-index">06 / OPERATING BOUNDARY</span><h2>选型边界</h2></div></div><div class="boundary"><article><h3><b>Private Flash High</b></h3><p>私密代码、内网数据、稳定日常 SQL/Python/故障诊断。Private Max 为 87.5%，未超过 High，不建议默认启用。</p></article><article class="online"><h3><b>Online Flash Low</b></h3><p>允许出网的简单低延迟请求。吞吐更高，但承担动态 alias、网络和供应商调度风险。</p></article><article class="online"><h3><b>Online Pro High</b></h3><p>复杂任务、private 首次失败、需要更高一次成功率。质量收益约 6.9pp。</p></article><article class="online"><h3><b>Pro Max · 按请求</b></h3><p>仅用于极难任务。没有超过 high，token 与分钟级尾延迟显著增加。</p></article></div></div></section>

<section id="evidence"><div class="shell"><div class="section-head"><div><span class="section-index">07 / EVIDENCE</span><h2>方法与证据</h2></div></div><div class="evidence"><ul><li>Private Max 已完成两轮质量测试：87.5%，波动 9.8pp。</li><li>Portal access log 记录 108 次 private 质量请求，以及 4 次 private 性能请求。</li><li>Private 性能请求数量与 1 次预热加 3 次测量完全一致。</li><li>Pro 六个质量 treatment，共 108 请求。</li><li>0 HTTP/网络错误，0 空 final，0 length 截断。</li><li>Private Low 与 High 渲染 prompt 完全相同；只有 Max 是独立 effort。</li><li>Private Max 的串行 SSE 性能和独立 Agent 任务本轮未测。</li><li>Python final 在不可变 ECR sandbox 中重评分，原始证据不覆盖。</li><li>快速决策 benchmark：精度 n=2，Agent n=1，不声明统计显著性。</li></ul><div class="links"><a href="BENCHMARK-COMPARISON-20260817.md">完整 Markdown 报告 <span>↗</span></a><a href="data/deepseek-private-online-comparison-20260817.json">机器可读汇总 <span>↗</span></a><a href="https://github.com/johnymoo/Nvidia-DGX/pull/32">GitHub PR 32 <span>↗</span></a><a href="https://api-docs.deepseek.com/guides/thinking_mode">DeepSeek Thinking Mode <span>↗</span></a></div></div></div></section>

...
def public_runtime(value):
    protected_status = value.get("protected", {})
    protected = protected_status.get("observed", {})
    return {
        ...
        "protected": {"enabled": protected_status.get("enabled", True),
                      "matches": protected_status.get("matches", False),
                      "container_id": protected.get("container_id"),
                      "health": protected.get("health"),
                      "restart_count": protected.get("restart_count")},
    }
...
def run_case(base, root, template, case, profile, run_id, artifacts, ffmpeg, canary=False):
    ...
    sampler.start()
    deadline_sample = time.monotonic() + 5
    while not samples and time.monotonic() < deadline_sample:
        time.sleep(0.01)
    if not samples:
        stop_event.set()
        sampler.join(5)
        raise RuntimeError("resource sampler did not start before submission")
...
def run(args):
    ...
    subject = root / "execution/minimax-h3/weights-manifest.tsv"
    report = {
        ...
        "deployment": {"gitee_revision": "9f9eca9589d4b4c0a01a8081c8c4add279e18868",
                       ...
                       "weights_manifest_sha256": sha256(subject),
                       "runtime_before": initial_public},

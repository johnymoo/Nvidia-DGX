...
class OffloadingConnectorWorker:
    ...
    def get_finished(self, finished_req_ids: set[str]) -> tuple[set[str], set[str]]:
        ...
        KV-OFFLOAD-PB(failure-overlay): failed transfers no longer assert.
        Failed LOADS still emit finished_recving — the connector scheduler
        truncates num_computed_tokens to the pre-load level when it
        processes failed_jobs, so the base scheduler's promotion path
        (scheduler._update_waiting_for_remote_kv) caches only the valid
        prefix and the request is rescheduled as a cold recompute. Failed
        STORES are reported via failed_jobs only; the manager drops the
        keys and the connector re-offers the blocks on a later store pass.
        """
        ...
        for transfer_result in self.worker.get_finished():
            ...
            if not transfer_result.success:
                ...
                if req_id is not None:
                    logger.warning(
                        "Offloading load job %d failed for request %s; "
                    ...
                    logger.warning(
                        "Offloading store job %d failed; keys will be "
    ...
    def build_connector_worker_meta(self) -> OffloadingWorkerMetadata | None:
        """Return completed/failed transfer job IDs since the last call."""
        if (

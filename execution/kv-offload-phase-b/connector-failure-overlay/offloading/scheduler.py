...
class OffloadingConnectorScheduler:
    ...
    def update_connector_output(self, connector_output: KVConnectorOutput):
        ...
        # KV-OFFLOAD-PB(failure-overlay): merge completions and failures.
        # With TP > 1 a job may appear in BOTH maps (one rank succeeded,
        # another failed); pending_count drains on either signal and the
        # job is processed exactly once — as failed if ANY worker reported
        # failure (a rank-local bad file means the aggregate KV is unusable).
        job_counts: dict[int, int] = {}
        failed_jobs: set[int] = set()
        for job_id, count in meta.completed_jobs.items():
            assert count > 0
            job_counts[job_id] = job_counts.get(job_id, 0) + count
        ...
        for job_id, count in job_counts.items():
            ...
            success = job_id not in failed_jobs
            ...
            if job_status.is_store:
                ...
                if success:
                    ...
                    # Drop the manager entries so future lookups miss (the
                    # file is unreadable); tolerate managers without the
                    # success kwarg (e.g. the CPU tier).
                    try:
                        self.manager.complete_load(
                            job_status.keys, req_status.req_context,
                            success=False,
                    ...
                    # Degrade the request to cold recompute: truncate to the
                    # tokens computed before this load began. The base
                    # scheduler's promotion path
                    # (_update_waiting_for_remote_kv) then caches only this
                    # valid prefix and the request is rescheduled cold.
                    req = req_status.req
                    pre_load = req_status.num_locally_computed_tokens
                    if req.num_computed_tokens > pre_load:
                        logger.warning(
                            "OffloadingConnector: load job %d failed for "
                            "request %s; recomputing %d tokens",
                            job_id,
                            req.request_id,
                            req.num_computed_tokens - pre_load,
                        )
                        req.num_computed_tokens = pre_load
                if self._blocks_being_loaded:
